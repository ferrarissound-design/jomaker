...
function ensurePteroFrames(onReady = null) {
  ...
  if (pteroLoadStarted || typeof document === 'undefined' || typeof Image === 'undefined') return;

  pteroLoadStarted = true;
  const image = new Image();
  image.decoding = 'async';
  image.onload = () => {
    try {
      pteroFrames = normalizePteroFrames(image);
      for (const callback of pteroReadyCallbacks.splice(0)) callback(pteroFrames);
    } catch {
      pteroReadyCallbacks.length = 0;
    }
  };
  image.onerror = () => { pteroReadyCallbacks.length = 0; };
  image.src = PTERO_SHEET_SRC;
}
...
export function makeAnimeEnemy(view, type) {
  if (type !== 'flyingEnemy') return makeBaseAnimeEnemy(view, type);
...
export function animateAnimeEnemy(view, node, enemy, time) {
  if (enemy.type !== 'flyingEnemy') return animateBaseAnimeEnemy(view, node, enemy, time);
