...
export class GameEngine {
  constructor(stage) {
    ...
    for (const o of stage.objects) {
      if (['ground', 'block', 'platform'].includes(o.type)) {
        this.solids.set(`${o.x},${o.y}`, {
          ...o,
          x: o.x * TILE,
          y: o.y * TILE,
          w: TILE,
  ...
  move(body, dt) {
    ...
    for (const o of this.nearby(body)) {
      if (o.type !== 'platform' && overlaps(body, o)) {
    ...
    body.grounded = false;
    for (const o of this.nearby(body)) {
      if (!overlaps(body, o) || (o.type === 'platform' && (body.vy < 0 || bottom > o.y + 1))) continue;
      if (body.vy >= 0) {
        body.y = o.y - body.h;
        body.grounded = true;
      } else {
  ...
  step(dt, input) {
    ...
    this.buffer = input.jump ? 0.13 : Math.max(0, this.buffer - dt);
    this.coyote = p.grounded ? 0.11 : Math.max(0, this.coyote - dt);
    ...
    for (const o of this.stage.objects) {
      ...
      if (o.type === 'coin') this.coins.add(`${o.x},${o.y}`);
