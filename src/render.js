...
export function drawPart(ctx, type, x, y, size = TILE, time = 0, active = false) {
  ...
  if (type === 'ground' || type === 'block') {
    ...
    ctx.fill();
    ctx.fillStyle = type === 'ground' ? '#8bc8a0' : '#ffd28a';
  ...
  if (type === 'spring') {
    ...
    ctx.beginPath();
    ctx.roundRect(5, 34, 38, 10, 4);
    ctx.fill();
    ctx.strokeStyle = '#7f6256';
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(10, 34);
    ctx.lineTo(18, 22);
  ...
  if (type === 'coin') {
    ...
    ctx.lineWidth = 3;
    ctx.stroke();
    ctx.fillStyle = '#b38239';
    ctx.fillRect(22, 17, 4, 14);
  }
  ...
  if (type === 'spike') {
    for (let i = 0; i < 3; i++) {
      ctx.beginPath();
      ctx.moveTo(i * 16, 46);
      ctx.lineTo(i * 16 + 8, 10);
      ctx.lineTo(i * 16 + 16, 46);
      ctx.fill();
    }
  }
  ...
  if (type === 'enemy') {
    ctx.beginPath();
    ctx.roundRect(6, 13, 36, 32, 10);
    ctx.fill();
    ctx.fillStyle = '#243442';
    ctx.fillRect(13, 23, 6, 6);
    ctx.fillRect(29, 23, 6, 6);
    ctx.fillRect(10, 43, 9, 4);
    ctx.fillRect(29, 43, 9, 4);
  }
  ...
  if (type === 'checkpoint') {
    ...
    ctx.beginPath();
    ctx.arc(24, 18, 12 + (active ? Math.sin(time * 5) : 0), 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#fff';
  ...
  if (type === 'goal' || type === 'start') {
    ...
    ctx.fillText(type === 'goal' ? '★' : 'S', 17, 20);
  }
...
export function drawPlayer(ctx, p, time) {
  ...
  ctx.fillStyle = '#efb66f';
  ctx.fillRect(-3, 25, 34, 5);
  ctx.fillStyle = '#27565e';
  const step = p.grounded && p.vx ? Math.sin(time * 18) * 3 : 0;
  ctx.fillRect(3, 33, 8, 5 + step);
  ctx.fillRect(18, 33, 8, 5 - step);
  ctx.restore();
}
...
export function render(ctx, w, h, stage, camera, scale, editing, game, time, selected, cameraY = 0) {
  ...
  for (const o of stage.objects) {
    ...
    if (game && o.type === 'enemy') continue;
    if (game?.coins.has(`${o.x},${o.y}`)) continue;
    const active = o.type === 'checkpoint' && game?.checkpoint?.x === o.x && game?.checkpoint?.y === o.y;
